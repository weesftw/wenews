...
public class DefaultNewsRepository implements NewsRepository {
    ...
    public News get(String title) {
        ...
        try {
            if(document != null) {
                var map = mapper.readValue(document.toJson(), News.class);
                return News.builder()
    ...
    public List<News> getNews(String category, int size) {
        ...
        collection.find(and(eqIgn(category))).sort(new BasicDBObject("_id", -1)).limit(size).forEach((Consumer<? super Document>) doc -> {
            try {
                var news = mapper.readValue(doc.toJson(), News.class);
        ...
        try {
            if(!exists(createNewsRequest.getTitle())) {
                ...
                return News.builder()
                        ...
                        .urlToImage(createNewsRequest.getUrlToImage())
    ...
    private Bson eqIgn(String value) {
        String patternString = "(?i)^" + value + "$";
        var pattern = Pattern.compile(patternString, Pattern.CASE_INSENSITIVE);
        return Filters.regex("category", pattern);
    }
}
